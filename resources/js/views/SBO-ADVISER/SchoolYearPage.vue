  ...
  <div class=" py-6">
        ...
        <div>
          ...
          <div v-else class="grid grid-cols-3 gap-y-10 gap-x-8">
            <button class="rounded hover:bg-gray-100 shadow bg-white hover:scale-95 transition ease-in-out cursor-pointer"   v-for="batch in school_years" :key="batch" @click="goToPage(batch.id)">
        ...
        <YearCard :title=" `SY.${batch.from}- ${batch.to}`" >
          ...
          <div class="border-l-2 pl-3 py-1 font-rubik border-green-600  mt-2">
            <p tabindex="0" class="t1 focus:outline-none text-sm font-medium leading-none text-green-700 font-rubki mb-2 ">
              ...
              <div class="mb-2">
                  <p tabindex="0" class="t2 italic focus:outline-none text-xs  leading-none  ">
                      Maria Jate Aliza Terizae
                    </p>
                    <p tabindex="0" class="focus:outline-none text-xs   leading-none  ">
                        <span class="t3 text-green-700 font-rubik text-xs inline-block">Governor   </span>  
                    </p>
              </div>
